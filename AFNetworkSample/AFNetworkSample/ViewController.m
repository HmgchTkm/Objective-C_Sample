...
- (IBAction)tappedPostBtn:(UIButton *)sender {
  
  AFHTTPSessionManager *afHttpMng = [AFHTTPSessionManager manager];
  
  // 送信パラメータの形式をJSON形式に変換
  afHttpMng.requestSerializer = [AFJSONRequestSerializer serializer];
  
  // リクエスト先のURL
  NSString *url = @"https://private-139a2-hoge8.apiary-mock.com/questions";
  
  // パラメータ用のJSON
  NSDictionary *paraJsonDic = @{@"question":@"Favourite programming language?",@"choices":@"Swift"};
 
  // HTTPリクエスト(GET)
  [afHttpMng
   POST:url
   parameters:paraJsonDic
   progress:nil
   success:
   ^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseJson)
   {
     ...
     // Mantleで JSON→modelオブジェクトへの変換
     PostResponseModel *responseModel =[MTLJSONAdapter
                                    modelOfClass:PostResponseModel.class
                                    fromJSONDictionary:responseJson
                                    error:&mantleErr
                                    ];
     ...
     // model の値を 画面のoutputViewへ出力
     self.outputView.text = responseModel.choices;
     NSLog(@"%i",responseModel.votes);
